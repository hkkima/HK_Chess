...
describe('createInitialMatches', () => {
  it('16명 → 8매치, 크로스오버 1v9 · 2v10 … 8v16', () => {
    const e = seedEntrants(standings(16));
    const m = createInitialMatches('16강', e);
    expect(m).toHaveLength(8);
    expect([m[0].seedHigh, m[0].seedLow]).toEqual([1, 9]);
    expect([m[1].seedHigh, m[1].seedLow]).toEqual([2, 10]);
    expect([m[7].seedHigh, m[7].seedLow]).toEqual([8, 16]);
    // 모든 매치 시드차가 8로 균일
    expect(m.every((x) => x.seedLow - x.seedHigh === 8)).toBe(true);
  });
...
describe('createNextStageMatches — 시드 보호(재폴드)', () => {
  const playAllHigh = (matches) => matches.forEach((m) => (m.winnerId = m.playerHighId));
  const pairs = (matches) => matches.map((m) => [m.seedHigh, m.seedLow]);

  it('8강 1v8·2v7·3v6·4v5 → 4강 1v4·2v3 → 결승 1v2 (시드1·2는 결승에서만)', () => {
    const seedMap = new Map(seedEntrants(standings(16)).map((e) => [e.playerId, e.seed]));
    let matches = createInitialMatches('16강', seedEntrants(standings(16)));

    playAllHigh(matches);
    const qf = createNextStageMatches('8강', matches, seedMap);
    expect(pairs(qf)).toEqual([[1, 8], [2, 7], [3, 6], [4, 5]]);

    playAllHigh(qf);
    const sf = createNextStageMatches('4강', qf, seedMap);
    expect(pairs(sf)).toEqual([[1, 4], [2, 3]]);

    playAllHigh(sf);
    const fin = createNextStageMatches('결승', sf, seedMap);
    expect(pairs(fin)).toEqual([[1, 2]]);
  });
});
